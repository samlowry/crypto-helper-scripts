...
def calculate_investments(start_date, end_date=None, interval='1m', amount=1, ticker='BTC', ticker_id=None, delay=DEFAULT_API_REQUEST_DELAY):
    ...
    start_date = datetime.datetime.strptime(start_date, '%d-%m-%Y')  # Update date format to dd-mm-yyyy
    end_date = datetime.datetime.strptime(end_date, '%d-%m-%Y')  # Update date format to dd-mm-yyyy
    ...
    if response.status_code == 200:
        ...
        if prices:
            ...
            for i in range(len(prices)):
                ...
                current_date = datetime.datetime.fromtimestamp(timestamp // 1000)  # Convert from milliseconds to seconds
                if current_date.date() > last_purchase_date.date():
                    ...
                    if interval_type == 'days' and (current_date - start_date).days % interval_value == 0:
                        ...
                        total_crypto += crypto_amount
                        total_investment += amount
                        start_date = start_date + relativedelta(months=interval_value)
        else:
            print(f"Unable to retrieve prices for {ticker} in the specified range.")
    else:
        print(f"Unable to retrieve prices for {ticker} in the specified range.")
...
def main():
    parser = argparse.ArgumentParser(description='Calculate the result of dollar-cost averaging (DCA) into a specific cryptocurrency.')
    parser.add_argument('-s', '--start', help='Start date for the DCA strategy in dd-mm-yyyy format.', required=True)
    parser.add_argument('-e', '--end', help='End date for the DCA strategy in dd-mm-yyyy format. If not provided, the current date will be used.')
    parser.add_argument('-i', '--interval', help='Interval for each purchase in the DCA strategy. For example: 1m for monthly, 7d for weekly or 2w for bi-weekly.', required=True)
    parser.add_argument('-a', '--amount', help='Amount in USD for each purchase.', type=float, required=True)
    parser.add_argument('-t', '--ticker', help='Ticker of the cryptocurrency. If not provided, BTC will be used.', default='BTC')
    parser.add_argument('-d', '--delay', help='Delay in seconds between each API request.', type=int, default=DEFAULT_API_REQUEST_DELAY)
    ...
    total_crypto, total_investment = calculate_investments(args.start, args.end, args.interval, args.amount, args.ticker, ticker_id, args.delay)
    ...
    print(f"End Date: {args.end if args.end else 'Today (' + datetime.datetime.now().strftime('%d-%m-%Y') + ')'}")
    ...
    if price_of_investments:
        print(f"Price of investments in fiat today: ${price_of_investments:.2f}")
        print(f"Delta in percent between invested money and current price of investments: {delta_percent:.2f}%")
    else:
