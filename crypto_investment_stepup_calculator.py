...
def main():
    parser = argparse.ArgumentParser(description='Calculate the result of step-up investments into a specific cryptocurrency.')
    parser.add_argument('-s', '--start', help='Start date for the investment strategy in dd-mm-yyyy format.', required=True)
    parser.add_argument('-e', '--end', help='End date for the investment strategy in dd-mm-yyyy format. If not provided, the current date will be used.')
    parser.add_argument('-p', '--step_percentage', help='Step percentage for each purchase. For example, 1 for 1% step increase.', type=float, default=1)
    ...
    total_crypto, total_investment = calculate_investments(args.start, args.end, args.step_percentage, args.amount, args.ticker, ticker_id, args.delay)
